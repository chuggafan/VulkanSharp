    ...
    class Sample : Form
    {
        ...
        static Instance CreateInstance()
        {
            ...
            return new Instance(new InstanceCreateInfo
            {
                ...
                {
                    ApplicationName = "Vulkan Windows XLogo",
        ...
        SurfaceFormatKhr SelectFormat(PhysicalDevice physicalDevice, SurfaceKhr surface)
        {
            ...
            throw new Exception("didn't find the R8G8B8A8Unorm or B8g8r8a8Unorm format");
        }
        ...
        Framebuffer[] CreateFramebuffers(Vulkan.Image[] images, SurfaceFormatKhr surfaceFormat)
        {
        ...
        CommandBuffer[] CreateCommandBuffers(Vulkan.Image[] images, Framebuffer[] framebuffers, Pipeline pipeline, Vulkan.Buffer vertexBuffer, Vulkan.Buffer indexBuffer, uint indexLength)
        {
        ...
        RenderPass CreateRenderPass(SurfaceFormatKhr surfaceFormat)
        {
            var attDesc = new AttachmentDescription
            {
                ...
                FinalLayout = Vulkan.ImageLayout.ColorAttachmentOptimal
            };
        ...
        {
            byte[] bytes = System.IO.File.ReadAllBytes("..\\..\\Shaders\\" + name);

            return bytes;
        }

        Vulkan.Buffer CreateBuffer(PhysicalDevice physicalDevice, object values, BufferUsageFlags usageFlags, System.Type type)
        {
        ...
        Pipeline[] CreatePipelines()
        {
            ...
            var vertexShaderModule = device.CreateShaderModule(LoadResource("shader.vert.spv"));
            var fragmentShaderModule = device.CreateShaderModule(LoadResource("shader.frag.spv"));
        ...
        Vulkan.Buffer CreateUniformBuffer(PhysicalDevice physicalDevice)
        {
        ...
        {
            var curInstance = CreateInstance();
            Win32SurfaceCreateInfoKhr surfaceInfo = new Win32SurfaceCreateInfoKhr();
            surfaceInfo.Hwnd = this.Handle;
            surfaceInfo.Hinstance = System.Runtime.InteropServices.Marshal.GetHINSTANCE(System.Reflection.Assembly.GetAssembly(this.GetType()).GetModules()[0]);
            var surface = curInstance.CreateWin32SurfaceKHR(surfaceInfo);
            var devices = curInstance.EnumeratePhysicalDevices();
